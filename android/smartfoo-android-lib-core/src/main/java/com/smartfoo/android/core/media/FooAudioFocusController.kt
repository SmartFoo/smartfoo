...
class FooAudioFocusController private constructor() {
    companion object {
        private val TAG = FooLog.TAG(FooAudioFocusController::class)

        @JvmStatic
        val instance by lazy { FooAudioFocusController() }

        var VERBOSE = true

        private val audioFocusMap by lazy {
            FooReflection.mapConstants(AudioManager::class, "AUDIOFOCUS_" )
        }

        /**
         * Similar to hidden [AudioManager.audioFocusToString]
         */
        @JvmStatic
        fun audioFocusToString(focus: Int) =
            FooReflection.toString(audioFocusMap, focus)
    }
